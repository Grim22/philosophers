...
/*   Updated: 2020/09/16 11:59:05 by bbrunet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** ->This prevents outputs on stdout from being mixed: status will be print one after the other
** ->This also prevents that any status is displayed after the game has ended:
** 		When the game ends, stop_all is set to yes (after the last status (philoX dies or philoX has eaten) has been displayed)
** 		As stop_all is set inside print_status (check_stop function), it is protected by lock
** 		A check on stop_all is performed at the begining of print_status. If stop_all = yes, print_status returns (no status is displayed)
*/
